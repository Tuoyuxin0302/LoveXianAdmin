layui.define(['flow','form','layer','upload','lovexian','element'],function(exports){
    ...
    // var location;
    // var typeId;
    // form.on('select(chart-location)',function (data) {
    //     location = $('.chartLocation').val();
    //     alert(location);
    //     form.render('select');
    // });
    // form.on('select(chart-type)',function (data) {
    //     typeId = $('.chartType').val();
    //     alert(typeId);
    //     form.render('select');
    // });
    //图片上传
    upload.render({
        elem: '.uploadNewImg',
        url: proPath+'/admin/shufflingFigure/uploadImages',
        //url: proPath+'/admin/shufflingFigure/uploadImages'+'?location='+location+'&typeId='+typeId,
        multiple: true,
    ...
    $("body").on("click",".img_del",function(){
        ...
        lovexian.modal.confirm('删除图片','确定删除图片吗？',function(){
            lovexian.del(proPath+'/admin/shufflingFigure/deleteImage',{"id":id},function (res) {
                if(res.status == '200'){
    ...
    $(".batchDel").click(function(){
        ...
        if($checkbox.is(":checked")){
            lovexian.modal.confirm('批量删除图片','确定删除选中的图片？',function(){
                ...
                lovexian.del(proPath+"/admin/scene/deleteImages",{"ids":ids.join(",")},function(res){
                    console.log(res);
