layui.define(['flow','form','layer','upload','lovexian','element'],function(exports){
    ...
    flow.load({
        elem: '#Images', //流加载容器
            ...
            lovexian.get(proPath+'/admin/scene/images',{'sceneId':$('#sceneId').val(),'pageSize':imgNums,'pageNum':page},function (res) {
    ...
    upload.render({
        elem: '.uploadNewImg',
        url: proPath+'/admin/scene/uploadImages',
        multiple: true,
    ...
    $("body").on("click",".img_del",function(){
        ...
        lovexian.modal.confirm('删除图片','确定删除图片吗？',function(){
            lovexian.del(proPath+'/admin/scene/deleteImage',{"id":id},function (res) {
                if(res.status == '200'){
    ...
    //对外暴露的接口
    exports('theme/culture/sceneImages', {});
