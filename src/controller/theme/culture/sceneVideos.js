...
function retryDecode(videoId){
    layui.use(['lovexian','baseSetting'],function () {
        var lovexian = layui.lovexian,
            ...
            $ = layui.jquery;
        lovexian.get(proPath+"/fileupload/videoProcess",{"id":videoId},function (res) {
            if(res.status == '100'){
...
layui.define(['flow','form','layer','upload','lovexian','element','baseSetting','admin','view'],function(exports){
    ...
    flow.load({
        elem: '#Videos', //流加载容器
            ...
            lovexian.get(proPath+'/admin/scene/videos',{'sceneId':$('#sceneId').val(),'pageSize':videoNums,'pageNum':page},function (res) {
                ...
                setTimeout(function(){
                    ...
                    $('.mask,.ico_play').click(function () {
                        ...
                        lovexian.get(proPath+"/admin/sceneVideo/getById",{"id":id},function (res) {
                            if(res.status == '400'){
                    ...
                    layui.use(['form', 'upload'], function() {
                        ...
                        upload.render({
                            ...
                            url: proPath+'/fileupload/uploadSceneImage',
                            headers:{
    ...
    upload.render({
        elem: '.uploadNewVideo',
        url: proPath+'/fileupload/video',
        data:{"cid":layui.data(setter.cid)[setter.cid],"sceneId":$('#sceneId').val()},
        headers:{
            Authentication :layui.data(setter.tableName)[setter.TOKENNAME]
        },
        exts: 'mp4',
    ...
    $("body").on("click",".img_del",function(){
        ...
        lovexian.modal.confirm('删除视频','确定删除视频吗？',function(){
            lovexian.del(proPath+'/admin/scene/deleteVideo',{"id":id},function (res) {
                if(res.status == '200'){
    ...
    //对外暴露的接口
    exports('theme/culture/sceneVideos', {});
